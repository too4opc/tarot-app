...
// สลับชุดประโยคหลายแบบต่อ field เพื่อลดความรู้สึก "เติมคำในช่องว่าง" เวลาคีย์เวิร์ดของไพ่ 2 ใบดันคล้ายกัน
const FALLBACK_DO_PHRASES = [
  'ลองเปิดใจรับสัญญาณตรงนี้ดูสักหน่อย แล้วให้เวลากับมันบ้าง',
  'ลองหยุดฟังเสียงข้างในสักครู่ ก่อนจะตัดสินใจอะไรต่อ',
  'ลองปล่อยให้ตัวเองสังเกตความรู้สึกนี้อย่างตรงไปตรงมา',
];
const FALLBACK_DONT_PHRASES = [
  'อย่าเพิ่งด่วนสรุปหรือมองข้ามความรู้สึกที่ผุดขึ้นมาตอนนี้',
  'อย่าฝืนเร่งทุกอย่างให้จบเร็วเกินไปจนลืมฟังใจตัวเอง',
  'อย่าปล่อยให้ความกังวลมาบดบังสิ่งที่กำลังค่อยๆ คลี่คลายอยู่',
];

const buildFallbackReading = (meaning, seed = 0) => {
  ...
  return {
    hook: `ไพ่ใบนี้กำลังพูดถึงเรื่อง${k1} ที่กำลังเคลื่อนไหวอยู่ในชีวิตคุณตอนนี้พอดี`,
    key1_title: k1,
    key1_desc: `เป็นแรงหลักที่กำลังทำงานอยู่เบื้องหลังสถานการณ์ของคุณตอนนี้ ลองสังเกตดูว่ามันแสดงตัวออกมาตรงไหนบ้าง`,
    key2_title: k2,
    key2_desc: `เป็นอีกแรงหนึ่งที่ประกอบกันอยู่ในเวลาเดียวกัน ทำให้เรื่องราวของคุณมีหลายมิติกว่าที่เห็นตอนแรก`,
    do: pick(FALLBACK_DO_PHRASES),
    dont: pick(FALLBACK_DONT_PHRASES),
    conclusion: `เมื่อคุณเข้าใจพลังของ${k3}มากขึ้น เส้นทางข้างหน้าจะค่อยๆ ชัดเจนขึ้นเองครับ`
  };
};
...
export default function App() {
  ...
  const generateAllAIReadings = useCallback(async (cards) => {
    ...
    try {
      ...
      safeSetState(genId, prev => ({
        ...prev,
        aiReadings: cards.map((c, i) => buildFallbackReading(c.meaning, i)),
